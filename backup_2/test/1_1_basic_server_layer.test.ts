...
function get_framework(){
    let framework = (new Framework())
        .add_organizational_layer('institution', z.object({name: z.string()}), z.object({}))
...
describe('Basic server layer test', function () {
    const port = 4601;
    ...
    it('should fetch based on an empty query', async function () {
        ...
        let person_response = await got.get(`http://localhost:${port}/api/test_institution/basic_collection`).json()
    ...
    it('should fetch based on ID', async function () {
        ...
        let person_response = await got.get(`http://localhost:${port}/api/test_institution/basic_collection/${created_person._id}`).json()
        assert.deepEqual(created_person, person_response)
    });

    it('should create a document', async function () {
        let person_response = await got.post(`http://localhost:${port}/api/test_institution/basic_collection`, {
            json: {
                age: 29,
                is_todd: false,
                date_deceased: new Date(),
